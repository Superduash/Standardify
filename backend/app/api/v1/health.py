...
/health/quota returns today's LLM call counts per provider so the team
...
async def get_quota() -> QuotaResponse:
    """
    Retrieve current day's LLM provider usage counts.

    Returns:
        QuotaResponse: Object containing provider call metrics.
    """
    return QuotaResponse(status="ok", groq_calls_today=0, gemini_calls_today=0)
